...
function getCard() {
    var cards = [];
    // 開発者向けAPIキー
    var trelloKey = "a34bd350b23126c75d16064ee74e3b65"; // ★★★
    // 開発者トークン
    var trelloToken = "c8ef8722ace08febb59b29affbb2fc6660c6396a6dcc53b2b97dc7637a61c0dd"; // ★★★
    // リストID
    ...
    for (var i = 0; i < json.length; i++) {
        var number = i+1;　// ナンバーを定義　=iにすると0になるため+1
        ...
        // カード情報を整形する
        var card = [number,name, idMembers, membersfullName.join(" - "), shortUrl]; //　配列の最初にnumberを追加
        // 配列の末尾に追加する
        cards.push(card);
    }
    // スプレッドシートに書き出し
...
function setSpreadsheet(cards) {
    ...
    sheet.getRange(2, 1, cards.length, cards[0].length).setValues(cards); //　列番号を2→1に変更　A列2行目から
...
function clear() {
    ...
    sheet.getRange(2, 1, 20, 6).clear() // clearを実行しても消えない部分があったため列数を4→6に変更
}